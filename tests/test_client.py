...
async def test_create_post_title_only(client):
    created = {**DRAFT_POST, "title": "New Post", "slug": "new-post"}
    with respx.mock(base_url=BASE) as mock:
        mock.post("/posts/").mock(return_value=httpx.Response(201, json=created))
...
async def test_create_post_with_all_fields(client):
    ...
    created = {**payload, "slug": "full-post", "url": "https://u.mataroa.blog/blog/full-post/"}
    with respx.mock(base_url=BASE) as mock:
        route = mock.post("/posts/").mock(return_value=httpx.Response(201, json=created))
...
async def test_update_post_body_only(client):
    updated = {**PUBLISHED_POST, "body": "Updated body."}
    with respx.mock(base_url=BASE) as mock:
...
async def test_update_post_slug_change(client):
    updated = {**PUBLISHED_POST, "slug": "hello-world-v2"}
    with respx.mock(base_url=BASE) as mock:
        mock.patch("/posts/hello-world/").mock(return_value=httpx.Response(200, json=updated))
...
async def test_delete_post(client):
    with respx.mock(base_url=BASE) as mock:
        mock.delete("/posts/hello-world/").mock(return_value=httpx.Response(204))
...
async def test_approve_comment(client):
    approved = {**SAMPLE_COMMENT, "is_approved": True}
    with respx.mock(base_url=BASE) as mock:
...
async def test_delete_comment(client):
    with respx.mock(base_url=BASE) as mock:
        mock.delete("/comments/43/").mock(return_value=httpx.Response(204))
...
async def test_create_page(client):
    with respx.mock(base_url=BASE) as mock:
        mock.post("/pages/").mock(return_value=httpx.Response(201, json=SAMPLE_PAGE))
...
async def test_update_page(client):
    updated = {**SAMPLE_PAGE, "body": "New bio.", "is_hidden": True}
    with respx.mock(base_url=BASE) as mock:
...
async def test_delete_page(client):
    with respx.mock(base_url=BASE) as mock:
        mock.delete("/pages/about/").mock(return_value=httpx.Response(204))
